//import { dnsResolve, isValidIpAddress } from '../src/3rd/mozillaPACruntime';
const r = require('../src/3rd/mozillaPACruntime'); // eslint-disable-line @typescript-eslint/no-var-requires
describe('Mozilla firefox PAC runtime test', () => {
	test('emulated native functions', () => {
		expect(r.isValidIpAddress(r.dnsResolve('www.example.com'))).toBe(true);
	...
	test('localHostOrDomainIs', () => {
		...
		expect(r.localHostOrDomainIs('www.google.com', 'www.mozilla.org')).toBe(
			false
		);
		expect(
	...
	test('isInNet', () => {
		expect(r.isInNet('www.mozilla.org', '173.245.1.1', '255.255.0.0')).toBe(
			true
		);
		...
		expect(r.isInNet('www.mozilla.org', '999.245.1.1', '255.255.0.0')).toBe(
			false
		);
	});
	...
	test('date times', () => {
		...
		const fakeDate = jest.fn((...params) =>
			params.length > 0 && params.length < 7
				? new realDate()
				: new realDate(tptr)
		);
		...
		expect(r.dateRange('JUL', 'JAN')).toBe(true);
